...
class RDSDashboard:
    ...
    def load_data(self):
        ...
        try:
            ...
            lightspeed_df = self.load_lightspeed_data()
            
            # Load Acumatica data
            acumatica_df = self.load_acumatica_data()
            ...
            if not self.df.empty:
                ...
                print(f"Loaded {len(self.df)} records from RDS")
            else:
    ...
    def load_lightspeed_data(self):
        """Load Lightspeed data from RDS"""
        try:
            ...
            conn.close()
            print(f"Loaded {len(df)} Lightspeed records")
            return df
    ...
    def load_acumatica_data(self):
        """Load Acumatica data from RDS"""
        try:
            conn = self.get_rds_connection()
            if not conn:
                return pd.DataFrame()
            ...
            df = pd.read_sql(query, conn)
            conn.close()
            print(f"Loaded {len(df)} Acumatica records")
            return df
